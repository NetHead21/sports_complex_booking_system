...
class TableFormatter:
    ...
    def format_table(self, data: List[Tuple[Any, ...]], title: Optional[str] = None) -> str:
        """
        ...
        # Add header
        header_line = " | ".join(header.ljust(column_widths[i]) for i, header in enumerate(self.headers))
...
def format_table_generic(data: List[Tuple], headers: List[str], title: str, column_formatters: Optional[Dict[int, Callable]] = None) -> str:
    """
...
def format_member_table(member_data: List[Tuple[str, str, float]], title: str = "🏟️ Sports Complex Members") -> str:
    """
...
def format_booking_table(booking_data: List[Tuple[str, str, str, str, str]], title: str = "🏨 Room Bookings") -> str:
    """
    ...
    return format_table_generic(
        ...
        title,
        {
            2: lambda x: x.strftime("%Y-%m-%d %H:%M") if hasattr(x, 'strftime') else str(x),  # Format datetime
            4: lambda x: "✅ PAID" if str(x).upper() == "PAID" else "❌ UNPAID"  # Format payment status
        }
    )
