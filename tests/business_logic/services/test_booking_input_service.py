...
class TestBookingInputServiceCollectRoomId(unittest.TestCase):
    ...
    @patch("business_logic.services.booking_input_service.get_user_input")
    def test_collect_room_id_too_long(self, mock_input):
        """Test validation of room ID maximum length (10 characters)."""

        mock_input.side_effect = ["VERYLONGID123", "T1"]
...
class TestBookingInputServiceCollectRoomType(unittest.TestCase):
    ...
    @patch("business_logic.services.booking_input_service.get_user_input")
    def test_collect_room_type_archery_range(self, mock_input):
        """Test selection of Archery Range."""

        mock_input.return_value = "3"

        result = BookingInputService._collect_room_type()

        self.assertEqual(result, "Archery Range")
